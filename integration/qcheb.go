...
func Qcheb(f gsl.Function, a, b float64, cheb12, cheb24 []float64) {
	fval := make([]float64, 25)
	v := make([]float64, 12)

	/* These are the values of cos(pi*k/24) for k=1..11 needed for the
	   Chebyshev expansion of f(x) */

	x := [11]float64{0.9914448613738104,
		0.9659258262890683,
		0.9238795325112868,
		0.8660254037844386,
		0.7933533402912352,
	...
	fval[0] = 0.5 * f.Evaluate(b)
	fval[12] = f.Evaluate(center)
	fval[24] = 0.5 * f.Evaluate(a)

	for i := 1; i < 12; i++ {
		j := 24 - i
		u := half_length * x[i-1]
		fval[i] = f.Evaluate(center + u)
		fval[j] = f.Evaluate(center - u)
	}
