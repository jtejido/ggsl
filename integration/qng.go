...
func Qng(f gsl.Function, a, b, epsabs, epsrel float64, result, abserr *float64, neval *int) err.GSLError {
	var (
	...
	if epsabs <= 0 && (epsrel < 0.5e-28 || epsrel < 50*gsl.Float64Eps) {
		*result = 0.0
		*abserr = 0.0
		*neval = 0
	...
	*neval = 87
	...
	return err.ERROR("failed to reach tolerance with highest-order rule", err.ETOL)
}
