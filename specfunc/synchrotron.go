...
var (
	...
	synchrotron1_cs = &chebyshevSeries{
		synchrotron1_data,
		12,
		-1.0, 1.0,
		9,
	...
	synchrotron2_cs = &chebyshevSeries{
		synchrotron2_data,
		11,
		-1.0, 1.0,
		7,
	...
	synchrotron1a_cs = &chebyshevSeries{
		synchrotron1a_data,
		22,
		-1.0, 1.0,
		11,
	...
	synchrotron21_cs = &chebyshevSeries{
		synchrotron21_data,
		12,
		-1.0, 1.0,
		9,
	...
	synchrotron22_cs = &chebyshevSeries{
		synchrotron22_data,
		12,
		-1.0, 1.0,
		8,
	...
	synchrotron2a_cs = &chebyshevSeries{
		synchrotron2a_data,
		16,
		-1.0, 1.0,
		8,
...
func Synchrotron_1_e(x float64, result *Result) err.GSLError {
	if x < 0.0 {
		...
		/* BJG: added first order correction term.  The taylor series
		   is  S1(x) = ((4pi)/(math.Sqrt(3)gamma(1/3))) * (x/2)^(1/3)
		   * (1 - (gamma(1/3)/2)*(x/2)^2/3 + (3/4) * (x/2)^2 ....) */
		z := math.Pow(x, 1.0/3.0)
		...
		c0 := gsl.Pi / gsl.Sqrt3
		px := math.Pow(x, 1.0/3.0)
		px11 := Pow_int(px, 11)
		t := x*x/8.0 - 1.0
		var result_c1, result_c2 Result
		synchrotron1_cs.Evaluate(t, &result_c1)
		synchrotron2_cs.Evaluate(t, &result_c2)
		result.val = px*result_c1.val - px11*result_c2.val - c0*x
		result.err = px*result_c1.err + px11*result_c2.err + c0*x*gsl.Float64Eps
		result.err += 2.0 * gsl.Float64Eps * math.Abs(result.val)
		return nil
	} else if x < -8.0*gsl.LnMinFloat64/7.0 {
		c0 := 0.2257913526447274323630976 /* log(math.Sqrt(pi/2)) */
		t := (12.0 - x) / (x + 4.0)
		var result_c1 Result
		synchrotron1a_cs.Evaluate(t, &result_c1)
		result.val = math.Sqrt(x) * result_c1.val * math.Exp(c0-x)
		result.err = 2.0 * gsl.Float64Eps * result.val * (math.Abs(c0-x) + 1.0)
		return nil
	} else {
...
func Synchrotron_2_e(x float64, result *Result) err.GSLError {
	if x < 0.0 {
		return DomainError(result)
	} else if x < 2.0*gsl.Sqrt2*gsl.SqrtFloat64Eps {
		/* BJG: added first order correction term.  The taylor series
		   is  S2(x) = ((2pi)/(math.Sqrt(3)*gamma(1/3))) * (x/2)^(1/3)
		   * (1 - (gamma(1/3)/gamma(4/3))*(x/2)^(4/3) + (gamma(1/3)/gamma(4/3))*(x/2)^2...) */

		z := math.Pow(x, 1.0/3.0)
		cf := 1 - 1.17767156510235e+00*z*x
		...
		px5 := Pow_int(px, 5)
		t := x*x/8.0 - 1.0
		var cheb1, cheb2 Result
		synchrotron21_cs.Evaluate(t, &cheb1)
		synchrotron22_cs.Evaluate(t, &cheb2)
		result.val = px*cheb1.val - px5*cheb2.val
		result.err = px*cheb1.err + px5*cheb2.err
		result.err += 2.0 * gsl.Float64Eps * math.Abs(result.val)
		return nil
	} else if x < -8.0*gsl.LnMinFloat64/7.0 {
		c0 := 0.22579135264472743236 /* log(math.Sqrt(pi/2)) */
		t := (10.0 - x) / (x + 2.0)
		var cheb1 Result
		synchrotron2a_cs.Evaluate(t, &cheb1)
		result.val = math.Sqrt(x) * math.Exp(c0-x) * cheb1.val
		result.err = gsl.Float64Eps * result.val * (math.Abs(c0-x) + 1.0)
		return nil
	} else {
		return UnderflowError(result)
	}
...
func Synchrotron_1(x float64) float64 {
	result := new(Result)
	status := Synchrotron_1_e(x, result)
	return EvalResult(result, status)
}

func Synchrotron_2(x float64) float64 {
	result := new(Result)
	status := Synchrotron_2_e(x, result)
	return EvalResult(result, status)
}
