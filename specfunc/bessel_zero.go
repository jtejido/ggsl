 ...
 * sizeof(c) = N+1
 */
func clenshaw(c []float64, N int, u float64) float64 {
	B_np1 := 0.0
	B_n := c[N]
	var B_nm1 float64
	var n int
 ...
 */
func mcmahon_correction(mu, beta float64) float64 {
	eb := 8.0 * beta
	ebsq := eb * eb

	if mu < gsl.Float64Eps {
		/* Prevent division by zero below. */
		var (
			term1 = 1.0 / ebsq
			term2 = -4.0 * 31.0 / (3 * ebsq * ebsq)
			term3 = 32.0 * 3779.0 / (15.0 * ebsq * ebsq * ebsq)
			term4 = -64.0 * 6277237.0 / (105.0 * ebsq * ebsq * ebsq * ebsq)
			term5 = 512.0 * 2092163573.0 / (315.0 * ebsq * ebsq * ebsq * ebsq * ebsq)
		)
		...
		var (
			mi    = 1.0 / mu
			r     = mu / ebsq
			n2    = 4.0 / 3.0 * (7.0 - 31.0*mi)
			n3    = 32.0 / 15.0 * (83.0 + (-982.0+3779.0*mi)*mi)
			n4    = 64.0 / 105.0 * (6949.0 + (-153855.0+(1585743.0-6277237.0*mi)*mi)*mi)
			n5    = 512.0 / 315.0 * (70197.0 + (-2479316.0+(48010494.0+(-512062548.0+2092163573.0*mi)*mi)*mi)*mi)
			n6    = 2048.0 / 3465.0 * (5592657.0 + (-287149133.0+(8903961290.0+(-179289628602.0+(1982611456181.0-8249725736393.0*mi)*mi)*mi)*mi)*mi)
			term1 = (1.0 - mi) * r
			term2 = term1 * n2 * r
			term3 = term1 * n3 * r * r
			term4 = term1 * n4 * r * r * r
			term5 = term1 * n5 * r * r * r * r
			term6 = term1 * n6 * r * r * r * r * r
		)
...
/* Assumes z >= 1.0 */
func olver_b0(z, minus_zeta float64) float64 {
	if z < 1.02 {
		var (
			a  = 1.0 - z
...
func olver_f1(z, minus_zeta float64) float64 {
	b0 := olver_b0(z, minus_zeta)
	h2 := math.Sqrt(4.0 * minus_zeta / (z*z - 1.0)) /* FIXME */
	return 0.5 * z * h2 * b0
}

func Bessel_zero_J0_e(s uint, result *Result) err.GSLError {
	if s == 0 {
		result.val = 0.0
		result.err = 0.0
		return err.ERROR("error", err.EINVAL)
	} else {
		/* See [F. Lether, J. Comp. Appl .Math. 67, 167 (1996)]. */
		var (
			P = []float64{1567450796.0 / 12539606369.0,
				8903660.0 / 2365861.0,
				10747040.0 / 536751.0,
			...
			beta   = (float64(s) - 0.25) * gsl.Pi
			bi2    = 1.0 / (beta * beta)
			R33num = P[0] + bi2*(P[1]+bi2*(P[2]+P[3]*bi2))
			R33den = Q[0] + bi2*(Q[1]+bi2*(Q[2]+Q[3]*bi2))
			R33    = R33num / R33den
		)
		result.val = beta + R33/beta
		result.err = math.Abs(3.0e-15 * result.val)
		return nil
	}
}

func Bessel_zero_J1_e(s uint, result *Result) err.GSLError {
	if s == 0 {
		result.val = 0.0
		result.err = 0.0
		return nil
		...
		var (
			...
			beta = (float64(s) + 0.25) * gsl.Pi
			bi2  = 1.0 / (beta * beta)
			Rnum = a[3] + bi2*(a[2]+bi2*(a[1]+bi2*a[0]))
			Rden = b[3] + bi2*(b[2]+bi2*(b[1]+bi2*b[0]))
			R    = Rnum / Rden
		)
		result.val = beta * (1.0 + R*bi2)
		result.err = math.Abs(2.0e-14 * result.val)
		return nil
	}
}

func Bessel_zero_Jnu_e(nu float64, s uint, result *Result) err.GSLError {
	fs := float64(s)
	if nu <= -1.0 {
		...
		result.err = 0.0
		if nu == 0.0 {
			return err.ERROR("no zero-th root for nu = 0.0", err.EINVAL)
		}
		return nil
	} else if nu < 0.0 {
		...
		result.val = 0.0
		result.err = 0.0
		return err.ERROR("unimplemented", err.EUNIMPL)
	} else if s == 1 {
		...
		beta := (fs + 0.5*nu - 0.25) * gsl.Pi
		mc := mcmahon_correction(4.0*nu*nu, beta)
		var rat12 Result
		Pow_int_e(nu/beta, 14, &rat12)
		result.val = beta * mc
		result.err = 4.0 * math.Abs(beta) * rat12.val
		result.err += 4.0 * math.Abs(gsl.Float64Eps*result.val)
		return nil
	} else {
		/* Olver uniform asymptotic. */
		var (
			as         Result
			stat_as    = Airy_zero_Ai_e(int(s), &as)
			minus_zeta = -math.Pow(nu, -2.0/3.0) * as.val
			z          = bessel_Olver_zofmzeta(minus_zeta)
...
func Bessel_zero_J0(s uint) float64 {
	result := new(Result)
	status := Bessel_zero_J0_e(s, result)
	return EvalResult(result, status)
}

func Bessel_zero_J1(s uint) float64 {
	result := new(Result)
	status := Bessel_zero_J1_e(s, result)
	return EvalResult(result, status)
}

func Bessel_zero_Jnu(nu float64, s uint) float64 {
	result := new(Result)
	status := Bessel_zero_Jnu_e(nu, s, result)
	return EvalResult(result, status)
}
