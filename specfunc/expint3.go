...
func Expint_3_e(x float64, result *Result) err.GSLError {
	val_infinity := 0.892979511569249211

	if x < 0.0 {
		return DomainError(result)
	} else if x < 1.6*gsl.Root3Float64Eps {
		result.val = x
		result.err = 0.0
		return nil
	} else if x <= 2.0 {
		t := x*x*x/4.0 - 1.0
		result_c := new(Result)
		expint3_cs.Evaluate(t, result_c)
		result.val = x * result_c.val
		result.err = x * result_c.err
		return nil
	} else if x < math.Pow(-gsl.LnFloat64Eps, 1.0/3.0) {
		t := 16.0/(x*x*x) - 1.0
		s := math.Exp(-x*x*x) / (3.0 * x * x)
		result_c := new(Result)
		expint3a_cs.Evaluate(t, result_c)
		result.val = val_infinity - result_c.val*s
		result.err = val_infinity*gsl.Float64Eps + s*result_c.err
		return nil
	} else {
		result.val = val_infinity
		result.err = val_infinity * gsl.Float64Eps
		return nil
	}
}

/*-*-*-*-*-*-*-*-*-* Functions w/ Natural Prototypes *-*-*-*-*-*-*-*-*-*-*/
func Expint_3(x float64) float64 {
	result := new(Result)
	status := Expint_3_e(x, result)
