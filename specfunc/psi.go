...
var psi_table = [PSI_TABLE_NMAX + 1]float64{
	0.0,
	-gsl.Euler,
...
var psi_1_table = [PSI_1_TABLE_NMAX + 1]float64{
	0.0,
	gsl.Pi * gsl.Pi / 6.0,
 ...
 * of the function
 */
func psi_x(x float64, result *Result) err.GSLError {
	y := math.Abs(x)
	...
	if x == 0.0 || x == -1.0 || x == -2.0 {
		...
		t := 8.0/(y*y) - 1.0
		result_c := new(Result)
		apsics.Evaluate(t, result_c)
		if x < 0.0 {
			s := math.Sin(gsl.Pi * x)
			c := math.Cos(gsl.Pi * x)
			if math.Abs(s) < 2.0*gsl.MinFloat64 {
				return DomainError(result)
			}

			result.val = math.Log(y) - 0.5/x + result_c.val - gsl.Pi*c/s
			result.err = gsl.Pi * math.Abs(x) * gsl.Float64Eps / (s * s)
	...
	if x < -1.0 {
		...
		t2 := 1.0 / (x + 1.0)
		t3 := 1.0 / v
		psics.Evaluate(2.0*v-1.0, result_c)

		result.val = -(t1 + t2 + t3) + result_c.val
		result.err = gsl.Float64Eps * (math.Abs(t1) + math.Abs(x/(t2*t2)) + math.Abs(x/(t3*t3)))
		result.err += result_c.err
		result.err += gsl.Float64Eps * math.Abs(result.val)
		return nil
	} else if x < 0.0 {
		...
		result.err = gsl.Float64Eps * (math.Abs(t1) + math.Abs(x/(t2*t2)))
		result.err += result_c.err
		result.err += gsl.Float64Eps * math.Abs(result.val)
		return nil
	} else if x < 1.0 {
		t1 := 1.0 / x
		psics.Evaluate(2.0*x-1.0, result_c)

		result.val = -t1 + result_c.val
		result.err = gsl.Float64Eps * t1
		result.err += result_c.err
		result.err += gsl.Float64Eps * math.Abs(result.val)
		return nil
	}
	...
	v := x - 1.0
	return psics.Evaluate(2.0*v-1.0, result)
}

// generic polygamma. assumes n >= 0 and x > 0
func psi_n_xg0(n int, x float64, result *Result) err.GSLError {
	if n == 0 {
	...
	stat_nf := Lnfact_e(uint(n), ln_nf)
	stat_e := Exp_mult_err_e(ln_nf.val, ln_nf.err, hzeta.val, hzeta.err, result)
	if gsl.IsEven(n) {
		result.val = -result.val
	}

	return err.ErrorSelect(stat_e, stat_nf, stat_hz)
}

/*-*-*-*-*-*-*-*-*-*-*-* Functions with Error Codes *-*-*-*-*-*-*-*-*-*-*-*/

// This computes the digamma function psi(n) for positive integer n. The digamma function is also called the Psi function.
func Psi_int_e(n int, result *Result) err.GSLError {
	if n <= 0 {
		return DomainError(result)
	...
	c4 := -1.0 / 252.0
	c5 := 1.0 / 240.0
	ni2 := (1.0 / float64(n)) * (1.0 / float64(n))
	ser := ni2 * (c2 + ni2*(c3+ni2*(c4+ni2*c5)))

	result.val = math.Log(float64(n)) - 0.5/float64(n) + ser
	result.err = gsl.Float64Eps * (math.Abs(math.Log(float64(n))) + math.Abs(0.5/float64(n)) + math.Abs(ser))
	result.err += gsl.Float64Eps * math.Abs(result.val)
	return nil
}

// This computes the digamma function psi(x) for general x, x != 0
func Psi_e(x float64, result *Result) err.GSLError {
	return psi_x(x, result)
}

func Psi_1piy_e(y float64, result *Result) err.GSLError {
	ay := math.Abs(y)
	...
	if ay > 1000.0 {
		...
		result.val = lny + sum
		result.err = 2.0 * gsl.Float64Eps * (math.Abs(lny) + math.Abs(sum))
		return nil
	} else if ay > 10.0 {
		/* [Abramowitz+Stegun, 6.3.19] */
		yi2 := 1.0 / (ay * ay)
		...
		result_c := new(Result)
		r1py_cs.Evaluate(x, result_c)
		result.val = result_c.val - gsl.Euler + v
		result.err = result_c.err
		result.err += 2.0 * gsl.Float64Eps * (math.Abs(v) + gsl.Euler + math.Abs(result_c.val))
		...
		for n := 1; n <= M; n++ {
			sum += 1.0 / (float64(n) * (float64(n)*float64(n) + y*y))
		}

		v := y2 * (sum + p)
		result.val = -gsl.Euler + v
		result.err = gsl.Float64Eps * (gsl.Euler + math.Abs(v))
		result.err += 2.0 * gsl.Float64Eps * math.Abs(result.val)
		return nil
	}
}

// This computes the Trigamma function psi'(n) for positive integer n.
func Psi_1_int_e(n int, result *Result) err.GSLError {
	if n <= 0 {
		return DomainError(result)
	...
	result.val = (1.0 + 0.5/float64(n) + 1.0/(6.0*float64(n)*float64(n)) + ser) / float64(n)
	result.err = gsl.Float64Eps * result.val
	return nil
}

// This computes the Trigamma function psi'(x) for general x.
func Psi_1_e(x float64, result *Result) err.GSLError {
	...
	if x == 0.0 || x == -1.0 || x == -2.0 {
		...
		for m := 0; float64(m) < M; m++ {
			sum += 1.0 / ((x + float64(m)) * (x + float64(m)))
		}

		stat_psi := psi_n_xg0(1, fx, result)
		result.val += sum
		result.err += M * gsl.Float64Eps * sum
		return stat_psi
	}

	sin_px := math.Sin(gsl.Pi * x)
	d := gsl.Pi * gsl.Pi / (sin_px * sin_px)
	r := new(Result)
	stat_psi := psi_n_xg0(1, 1.0-x, r)
	result.val = d - r.val
	result.err = r.err + 2.0*gsl.Float64Eps*d
	return stat_psi
}

// These routines compute the polygamma function psi^n (x) for n >= 0, x > 0.
func Psi_n_e(n int, x float64, result *Result) err.GSLError {
	if n == 0 {
		return Psi_e(x, result)
	} else if n == 1 {
...
func Psi_int(n int) float64 {
	result := new(Result)
	status := Psi_int_e(n, result)
	return EvalResult(result, status)
}

func Psi(x float64) float64 {
	result := new(Result)
	status := Psi_e(x, result)
	return EvalResult(result, status)
}

func Psi_1piy(x float64) float64 {
	result := new(Result)
	status := Psi_1piy_e(x, result)
	return EvalResult(result, status)
}

func Psi_1_int(n int) float64 {
	result := new(Result)
	status := Psi_1_int_e(n, result)
	return EvalResult(result, status)
}

func Psi_1(x float64) float64 {
	result := new(Result)
	status := Psi_1_e(x, result)
	return EvalResult(result, status)
}

func Psi_n(n int, x float64) float64 {
	result := new(Result)
	status := Psi_n_e(n, x, result)
	return EvalResult(result, status)
}
